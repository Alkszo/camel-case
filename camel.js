...
// so it's a handy method for checking if undesired characters ('-' and ' ' in this case) are eliminated
const toCamelCase = (str) => {
    while (str.indexOf('-') !== -1) { //this will run untill there are '-' in the string
        let index = str.indexOf('-') + 1; //this identifies the index of character following first dash in the string
        let upper = str[index].toUpperCase(); //this takes said character to upper case
        str = replaceChar(str, upper, index); //this uses previously written function to replace a character after dash with it's uppercase version
        str = str.replace('-', '_'); //this replaces first dash in the string with underscore; while loop will go over the string again and execute if there are any '_' left
    }
